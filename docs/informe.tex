...
\begin{tikzpicture}[node distance=10mm, every node/.style={draw, rounded corners, align=center, font=\small}]
    % Nodes
    \node (client) {Cliente /Aplicación Frontend};
    \node[right=of client] (api) {API REST (FastAPI, Uvicorn)};
    \node[right=of api] (model) {Modelo (\texttt{trained\_pipeline-0.1.0.pkl})};
    \node[below=of api] (prefect) {Orquestador (Prefect) - tareas de entrenamiento e inferencia};
    \node[below=of model] (tmp) {Almacenamiento Temporal (/tmp/bancox\_train \; parquet)};
    \node[right=of model] (db) {Base de Datos (MySQL) - tablas raw / prepared};
    \node[below=of tmp] (logs) {Logging (archivo rotativo, \texttt{LOG\_FILE})};
